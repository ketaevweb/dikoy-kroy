...
- **Промокоды и аналитика** — FIRST10 / DROP04VIP / FREESHIP (статический конфиг `src/lib/promo.ts`, серверный пересчёт скидки в чекауте), pop-up подписки (10 сек / exit-intent, промокод взамен почты), таймер до следующего дропа, «с этим часто берут», предзаказ «Узнать о поступлении» на распроданных моделях, SEO-страницы категорий, админка `/admin` (воронка конверсии, заказы, промо, подписчики) и цели Яндекс Метрики.
